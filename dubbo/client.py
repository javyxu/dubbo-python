...
class DubboClient(object):
    ...
    def __init__(self, interface, version='1.0.0', dubbo_version='2.7.6', nacos_register=None, zk_register=None, host=None):
        ...
        :param nacos_register: Nacos注册中心管理端，参见类：NacosRegister
        :param zk_register: zookeeper注册中心管理端，参见类：ZkRegister
        :param host: 远程主机地址，用于绕过zookeeper或Nacos进行直连，例如：172.21.4.98:20882
        """
        if not zk_register and not host and not nacos_register:
            raise RegisterException('zk_register、nacos_register和host至少需要填入一个')
...
class ZkRegister(object):
    ...
    def _get_providers_from_zk(self, path, interface):
        ...
        providers = [provider for provider in map(parse_url, providers) if provider['scheme'] == 'dubbo']
        if not providers:
            raise RegisterException('no providers for interface {}'.format(interface))
        self._register_consumer(providers)
        self.hosts[interface] = [provider['host'] for provider in providers]
...
class NacosRegister(object):
    """
    NacosRegister的主要作用：根据特定的interface从nacos中取出与之相关的所有provider的host并且监听provider的变化，当发生变化时更新最新的hosts到本地缓存；
    """

    def __init__(self, hosts, username=None, password=None, endpoint=None, namespace=None, timeout=None, namespace_id=None, group_name=None):
        """
    ...
    def get_provider_host(self, interface, version):
        """
        从nacos中可以根据服务名称获取到此服务某个provider的host
        :param interface:
        :return:
        """
        if interface not in self.hosts:
